...
@Component({
  ...
  styleUrls: ['./visitados-chart.component.css'],
})
export class VisitadosChartComponent implements OnInit {
  constructor(
    ...
    this.authService.obtenerDatosRestauranteToken().subscribe((resp: any) => {
      this.searchService.getAllRestaurantes().subscribe((resp1) => {
        const datos = resp1.restaurantes.map((restaurante: any) => {
          return [restaurante.vecesVisitado, restaurante.nombre];
        });
        ...
        this.barChartData.datasets[0].data = [
          ...
          topThree[1][0],
          topThree[2][0],
          resp.restaurante.vecesVisitado,
        ];
        this.chart?.update();
      });
    });
  ...
  public barChartData: ChartData<'bar'> = {
    labels: [],
    datasets: [
      { data: [], label: 'Número de visitas', backgroundColor: 'blue' },
    ],
  };
}
